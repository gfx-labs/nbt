...
type InvalidTypeError struct {
	...
	Child *InvalidTypeError
}

// Error ...
func (err InvalidTypeError) Error() string {
	s := fmt.Sprintf(
		"nbt: invalid type for tag '%v' at offset %v: cannot unmarshalTag %v into %v",
		err.Field,
		err.Off,
		tagName(err.TagType),
		err.FieldType,
...
func (err UnknownTagError) Error() string {
	return fmt.Sprintf(
		"nbt: unknown tag '%v' at offset %v during op '%v'",
		err.TagType,
		err.Off,
		err.Op,
	)
...
func (err UnexpectedTagError) Error() string {
	return fmt.Sprintf(
		"nbt: unexpected tag %v at offset %v: tag is not valid in its context",
		tagName(err.TagType),
		err.Off,
	)
}
...
func (err InvalidArraySizeError) Error() string {
	return fmt.Sprintf(
		"nbt: mismatched array size at %v during op '%v': expected size %v, found %v in NBT",
		err.Off,
		err.Op,
		err.GoLength,
		err.NBTLength,
...
func (err UnexpectedNamedTagError) Error() string {
	return fmt.Sprintf(
		"nbt: unexpected named tag '%v' with type %v at offset %v: not present in struct to be decoded into",
		err.TagName,
		tagName(err.TagType),
		err.Off,
	)
...
func (err FailedWriteError) Error() string {
	return fmt.Sprintf(
		"nbt: failed write during op '%v' at offset %v: %v",
		err.Op,
		err.Off,
		err.Err,
	)
...
func (err IncompatibleTypeError) Error() string {
	return fmt.Sprintf(
		"nbt: value type %v (%v) cannot be translated to an NBT tag",
		err.Type,
		err.ValueName,
	)
}
...
func (err InvalidStringError) Error() string {
	return fmt.Sprintf(
		"nbt: string at offset %v is not valid: %v (%v)",
		err.Off,
		err.Err,
		err.String,
	)
...
func (err MaximumBytesReadError) Error() string {
	return fmt.Sprintf(
		"nbt: limit of bytes read %v with NetworkLittleEndian format exhausted",
		maximumNetworkOffset,
	)
}
